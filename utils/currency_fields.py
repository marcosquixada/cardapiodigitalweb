...
class FloatPField(forms.RegexField):
    def __init__(self, regex=r'^\d{1,10}(,\d{1,2})?$', *args, **kwargs):
        super(FloatPField, self).__init__(regex, *args, **kwargs)

    def clean(self, value):
        u"""Troca vírgula por ponto após a validação"""
        value = super(FloatPField, self).clean(value)
    ...
    def widget_attrs(self, widget):
        return {'class': 'valor_float'}

class FloatPInput(forms.TextInput):
    class Media:
        js = (settings.MEDIA_URL+'/grappelli/js/widgets.js',)

    def render(self, name, value, attrs=None):
        u"""Troca vírgula por vazio e ponto por vírgula após a renderização"""
        attrs['class'] = ' '.join([attrs.get('class', ''), 'valor_float'])
        ret = super(FloatPInput, self).render(name, value, attrs)
