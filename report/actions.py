# -*- coding: utf-8 -*-
from html2report import html_report_generic_detailed, html_report_generic, html_report_pendencias
...
def report_generic(self, request, queryset):
        ...
        #list_report = []
        html = html_report_generic(get_header_detailed(self, queryset), self.list_report, queryset)
        #html = html_report_generic(get_header_detailed(self, queryset), self.fieldsets_report, query)
        pdf = pisa.CreatePDF(html, response)
        return response

report_generic.short_description = _(u"Imprimir relatório")


def report_pendencias(self, request, queryset):
        response = HttpResponse(mimetype='application/pdf')
        response['Content-Disposition'] = 'attachment; filename=%s.pdf' % (smart_str(queryset.model._meta.verbose_name.upper()))
        ...
        #campos = rtype.campos.replace('[', '').replace(']', '').replace('u\'', '').replace('\'', '').replace(',', '').split(' ')
        #list_report = []
        html = html_report_pendencias(get_header_detailed(self, queryset), self.list_report, queryset)
        #html = html_report_generic(get_header_detailed(self, queryset), self.fieldsets_report, query)
        pdf = pisa.CreatePDF(html, response)
        return response
report_pendencias.short_description = _(u"Imprimir relatório")
